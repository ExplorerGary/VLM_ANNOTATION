...
# 在VLM-AD中，structured prompt并没有添加reasoning的内容，我也认为这个东西可能和freedom的定位重复，因此我从structured prompt中删去了。
STRUCTURED_SYS_IMPROVED = (
    ...
    "which may include a gaze overlay showing where the driver is looking. "
    "Determine the optimal driving action by synthesizing visible environmental hazards "
    "and the driver's current gaze target (provided as ground truth). "
    "Use only evidence visible in the current frame. "
    "Do not add explanations or extra text."
)

STRUCTURED_USER_IMPROVED = (
    ""
    "Input Data:\n"
    "- Driver Gaze Target (Ground Truth): {gaze_target}\n\n" 
    "Task:\n"
    ...
    '{"workzone_presence":"<one option>","workzone_type":"<one option>","traffic_condition":"<one option>","primary_hazard":"<one option>","attention_alignment":"<one option>","risk_level":"<one option>","recommended_action":"<one option>"}'
)
#endregion
...
class PromptImproved(Prompt):
    def __init__(self, system_message:str = None, user_message:str = None, seed:str = ""):
        seed_upper = seed.upper() if seed else ""
        if seed_upper == "FREEDOM":
            system_message = FREEDOM_SYS_IMPROVED
            user_message = FREEDOM_USER_IMPROVED
        elif seed_upper == "STRUCTURED":
            system_message = STRUCTURED_SYS_IMPROVED
            user_message = STRUCTURED_USER_IMPROVED
